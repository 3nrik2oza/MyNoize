...
class MusicScreenViewModel(
    ...
    private fun generateRandomPlaylist(){
        ...
        viewModelScope.launch {
            val favoriteSongs = userRepository.user.first().favoriteSongs
            playlistRepository.getPlaylistsWithSongs(favoriteSongs).onSuccess { playlists ->
                val playlists = playlists.shuffled()
                _state.update { it.copy(playlistsForUser = playlists) }
            }
        }
    }
