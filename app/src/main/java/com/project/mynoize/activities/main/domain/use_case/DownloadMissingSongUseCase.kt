...
class DownloadMissingSongUseCase(
    ...
    suspend operator fun invoke(remoteSong: Song, albumImageUrl: String? = null): EmptyResult<SyncError>{
        var localSongUrl = ""
        storageRepository.downloadToLocalMemory(remoteSong.songUrl, "songs").onSuccess { localSongUrl = it }
        if(localSongUrl.isEmpty()) return Result.Error(SyncError.FirebaseStorageError)

        var imageUrl = albumImageUrl
        if(imageUrl == null){
            getImageUrl(albumId = remoteSong.albumId, artistId = remoteSong.artistId).onSuccess {
                imageUrl = it
